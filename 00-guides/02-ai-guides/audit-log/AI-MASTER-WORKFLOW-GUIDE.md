...
2. [AICQ_AUDIT_LOG_SCHEMA.md](./AICQ_AUDIT_LOG_SCHEMA.md) と [AICQ_Devin_reasoning_log.md](./AICQ_Devin_reasoning_log.md) を参照し、structured_outputスキーマ/Playbook を適用
...
[本ガイド: AI-MASTER-WORKFLOW-GUIDE.md]
    ↓ 統合ワークフロー・司令塔
    ├─→ DOCUMENT-USAGE-MANUAL.md (ドキュメント選択・使用方法)
    ├─→ ESCALATION-CRITERIA-GUIDE.md (エスカレーション判断)
...
### 監査ログ（structured_output）との連携

本ガイドの各Phaseは、**AI-WORKLOG-ENFORCEMENT-GUIDE.md** で定義された監査ログ運用（structured_output＋messages）と連携して使用します。

- **各フェーズにおいて、作業のたびに監査ログを記録すること（必須）。** 作業の重要・軽微を問わず、すべての作業を監査ログ記録の対象とする。フェーズ開始時・完了時には、AI-WORKLOG-ENFORCEMENT-GUIDE.md の「3.3 フェーズ／イベント別更新タイミング」に従い、structured_output を更新してください。
- フェーズ内で以下のイベントが発生した場合も、**その都度** structured_output を更新してください。重要なサブタスクの開始・完了時も漏らさず記録すること。
  - 新しい事実の取得
  - 仮説・前提（assumptions）の設定／変更
  - 設計・実装・テスト方針の重要な意思決定
  - ガイドラインからの逸脱の検討／決定／復帰

これにより、「どのフェーズで・どの意思決定が行われたか」が structured_output＋messages から一貫して追跡可能になります。

また、監査ログの生成・保存方法そのもの（START_SESSION / LOG_EVENT / END_SESSION の流れや必須成果物）は、  
`AI-AUDIT-LOG-PROTOCOL.md` に定義されたプロトコル仕様に従ってください。
...
- **Phase 2A (実装前設計)**: 実装開始前の軽量設計
  - 成果物: ADR、API契約書（**OpenAPI 3.0簡易版**）、制約条件文書
  - 目的: 実装の方向性を定める最小限の設計
  - 形式: OpenAPI 3.0（基本構造のみ、バリデーション等は省略）
  - 実行順序: Phase 1 → **Phase 2A** → Phase 3
  
- **Phase 5 (実装後設計)**: 実装完了後の詳細ドキュメント化
  - 成果物: 詳細設計書、完全版API仕様（**OpenAPI 3.0完全版**）、アーキテクチャ図
  - 目的: 保守・運用のための完全なドキュメント
  - 形式: OpenAPI 3.0（全バリデーション、全エラー、example完全版）
  - 実行順序: Phase 4 → **Phase 5** → Phase 5
...
#### タイプ2: 新規機能開発 (NFD - New Feature Development)
**特徴**: 既存プロジェクトに新しいユースケース・ビジネス機能を追加
```
...
#### タイプ3: 既存機能追加・改修 (ENH - Enhancement / Modification)
**特徴**: 既存機能の拡張・改善（新ユースケースは追加しない）
```
推奨フロー: Phase 0 → 2 → 3 → 4 → 5 (Phase 1 スキップ)
...
#### タイプ5: リファクタリング (REF - Refactoring)
**特徴**: 外部動作を変えず、コード品質の改善
```
...
#### タイプ6: 設計変更・アーキテクチャ改善 (ARC - Architecture Improvement)
**特徴**: システムの設計・アーキテクチャの根本的な見直し
```
...
#### タイプ7: 緊急修正 (HOT - Hotfix)
**特徴**: 24時間以内の対応が必要な本番障害の緊急対応
```
...
#### タイプ8: 実験的機能 (POC - Spike / POC)
**特徴**: 技術検証、実験、概念実証（本番導入は未定）
```
...
[Phase 1] → プロジェクト構造 + 技術選定書
  ↓
[Phase 2A] → ADR + API契約書 + 制約条件文書
  ↓
[Phase 3] → ソースコード + テストコード
  ↓
[Phase 4] → レビュー結果 + 品質レポート
  ↓
[Phase 5] → 詳細設計書 + 完全版API仕様 + アーキテクチャ図
  ↓
[Phase 5] → デプロイ設定 + 本番環境
  ↓
[Phase 6] → 運用ログ + 改善提案
...
> 🛡️ **監査ログ記録（必須）**: 本フェーズでは、**作業のたびに** 監査ログ（structured_output）を更新すること。フェーズ開始時・完了時に加え、重要なサブタスク・意思決定のたびに [AI-WORKLOG-ENFORCEMENT-GUIDE.md](./AI-WORKLOG-ENFORCEMENT-GUIDE.md) の「3.1 更新の必須タイミング」「3.3 フェーズ／イベント別更新タイミング」に従って記録し、漏れを防ぐこと。
...
#### 📚 Phase 0 成果物作成時の参照ドキュメント

**重要**: Phase 0で企画・要求定義を行う際は、以下のフォルダを必ず参照してください。
...
**参照タイミング**: Phase 0開始直後、プロジェクト憲章やBRD作成前

**詳細**: `AI-DELIVERABLE-REFERENCE-GUIDE.md` の Phase 0 セクションを参照
```
...
#### 📚 Phase 1 成果物作成時の参照ドキュメント

**重要**: Phase 1で要件定義を行う際は、以下のフォルダを必ず参照してください。
...
**参照タイミング**: Phase 1開始直後、各要件定義書作成前

**詳細**: `AI-DELIVERABLE-REFERENCE-GUIDE.md` の Phase 1 セクションを参照
...
> 🛡️ **監査ログ記録（必須）**: Phase 2〜6 の各フェーズでは、**作業のたびに** 監査ログ（structured_output）を更新すること。フェーズ開始時・完了時に加え、重要なサブタスク・意思決定のたびに [AI-WORKLOG-ENFORCEMENT-GUIDE.md](./AI-WORKLOG-ENFORCEMENT-GUIDE.md) の「3.1 更新の必須タイミング」「3.3 フェーズ／イベント別更新タイミング」に従って記録し、漏れを防ぐこと。
...
##### ⚠️ Phase 2A vs Phase 5 の成果物区別

Phase 2は2つのサブフェーズに分かれます：

**Phase 2A: 基本設計（軽量版・実装前）**
- **タイミング**: Phase 3（実装）の前
- **目的**: 実装可否判断、技術的実現性の確認
- **記載レベル**: 軽量版（必要最小限の設計情報のみ）
- **作成する成果物**:
  - システム構成図（主要コンポーネントのみ）
  - アーキテクチャ設計書（アーキテクチャパターンと主要技術のみ）
  - データベース設計書（主要エンティティとリレーションのみ）
  - 画面設計書（ワイヤーフレームレベル）
  - API設計書（主要エンドポイントのみ）

**Phase 5: 基本設計（完全版・実装後）**
- **タイミング**: Phase 3（実装）完了後
- **目的**: 実装結果を反映した完全な設計書の作成
- **記載レベル**: 完全版（詳細なパラメータ、設定値、実装詳細を含む）
- **作成する成果物**:
  - システム構成図（詳細なコンポーネント構成、ネットワーク設定、パラメータ）
  - アーキテクチャ設計書（詳細な技術仕様、ライブラリバージョン、設定値）
  - データベース設計書（全カラム詳細、制約、インデックス、パフォーマンス設定）
  - 画面設計書（詳細な画面仕様、寸法、カラーコード、フォント）
  - API設計書（全エンドポイント、リクエスト/レスポンス詳細、エラーコード）
  - セキュリティ設計書（認証・認可・暗号化・脆弱性対策の詳細）

**参照資料**: `AI-DELIVERABLE-REFERENCE-GUIDE.md` の Phase 2A/2.2 セクションを参照
...
- **参照**: `implementation-phase-document-reference-guide.md`, `03-development-process/feature-flag-management/` (機能フラグ利用時)
- **概要**: コード実装、テスト作成
...
> **詳細ガイド**: [ESCALATION-CRITERIA-GUIDE.md](ESCALATION-CRITERIA-GUIDE.md)  
...
| アクション | 記号 | 説明 |
|-----------|------|------|
| **即座に停止・エスカレーション** | 🔴 | 作業を停止し、人間の判断を待つ |
| **実装前エスカレーション** | 🟠 | Phase 2開始前に確認リクエストを作成 |
| **実装後レビュー要求** | 🟡 | Phase 4で人間のレビューを求める |
| **記録して進行** | 🟢 | 標準的な想定で進行、判断理由を記録 |
...
- [ ] セキュリティに重大な影響がある決定
  - 個人情報の取扱変更、認証方式の変更、暗号化方式の選定
- [ ] 組織標準から大きく逸脱する必要がある
  - 未承認技術の使用、規約の根本的変更提案
- [ ] 技術選定が組織方針と矛盾する
  - 新しいプログラミング言語・データベースの導入
- [ ] 複数ドキュメント間で解決不能な矛盾がある
  - 参照: `10-governance/standards-hierarchy-and-precedence.md`
- [ ] 予算・スケジュールに重大な影響がある
  - 見積工数の2倍以上増加、納期遵守が不可能
- [ ] PBIの解釈に重大な不確実性がある
  - アーキテクチャ・技術選定に影響する不明点

**判断のヒント**: [自己診断チェックリスト](ESCALATION-CRITERIA-GUIDE.md#提案4-エスカレーション要否の自己診断チェックリスト) でセクションA（セキュリティ）またはセクションB（アーキテクチャ）に2つ以上該当
...
**判断のヒント**: [自己診断チェックリスト](ESCALATION-CRITERIA-GUIDE.md#提案4-エスカレーション要否の自己診断チェックリスト) でセクションB（アーキテクチャ）に1つ、またはセクションC（ビジネスロジック）に2つ以上該当
...
- [ ] 複雑なビジネスロジックを実装した
- [ ] 複数の技術を組み合わせた複雑な実装
- [ ] データベースインデックスの追加（パフォーマンス最適化）

**判断のヒント**: [自己診断チェックリスト](ESCALATION-CRITERIA-GUIDE.md#提案4-エスカレーション要否の自己診断チェックリスト) でセクションC（ビジネスロジック）に1つ、またはセクションD（組織標準）に1つ以上該当
...
- [ ] UI/UX表示・デザインの微調整
- [ ] ログメッセージの改善
- [ ] 組織標準・ベストプラクティスが明確な実装
- [ ] 単一サービス内で完結する機能追加

**判断のヒント**: [自己診断チェックリスト](ESCALATION-CRITERIA-GUIDE.md#提案4-エスカレーション要否の自己診断チェックリスト) でセクションE（UI・ログ）に3つ以上該当 & セクションA-D該当なし
...
| 状況 | 参照先 |
|------|--------|
| ドキュメントの選び方が分からない | DOCUMENT-USAGE-MANUAL.md（第3章: フェーズ別ドキュメントマップ） |
| 判断に迷っている（エスカレーション要否） | ESCALATION-CRITERIA-GUIDE.md |
...
**関連ドキュメント**:
- [AI使用マニュアル](./DOCUMENT-USAGE-MANUAL.md) - このガイドの使い方・ドキュメント選択支援
- [エスカレーション基準ガイド](./ESCALATION-CRITERIA-GUIDE.md) - エスカレーション判断支援
