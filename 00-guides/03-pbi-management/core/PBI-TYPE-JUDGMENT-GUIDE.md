...
**状況:**
PBIに複数のタイプ要素が混在しており、どれを主タイプとすべきか判断が必要。
...
ELSE IF システムアーキテクチャの根本的な変更
  AND 複数のコンポーネントに影響
  AND Phase 2（設計）が必須
THEN
  → 「設計変更・アーキテクチャ改善（ARC）」
  → バグ修正やリファクタリングはARCの中で対応

ELSE IF 新しいユースケースの追加
  AND 新しいビジネス機能
THEN
  → 「新規機能開発（NFD）」
  → リファクタリングや小規模改善はNFDの中で対応
...
- 主タイプのPhase経路を基本とする
- 副次的要素は実装フェーズで併せて対応
- どうしても分離できない場合のみ複合型として扱う

**具体例と判定:**

| ケース | 要素 | 判定結果 | 理由 |
|-------|------|---------|------|
| 新機能+バグ修正 | NFD + BUG | **NFD** | 新ユースケース追加が主目的 |
| バグ修正+設計変更 | BUG + ARC | **ARC** | 根本的な設計変更が必要 |
| リファクタリング+機能改善 | REF + ENH | **ENH** | 機能変更を伴う |
| 緊急修正+根本対策 | HOT + ARC | **HOT** → 別PBI(**ARC**) | 緊急性優先、根本対策は別途 |
| POC検証後の実装 | POC + NFD | **NFD** | 本番品質が必要 |
| アーキテクチャ改善+新機能 | ARC + NFD | **ARC** | 設計変更が基盤となる |

**分割を検討すべきケース:**

以下の場合は、1つのPBIとせず、複数PBIに分割することを強く推奨します:
...
2. NPIと他の要素の組み合わせ（新規プロジェクト以外の追加要素）
   - NPI: 新規プロジェクト立ち上げ
   - 別PBI: 立ち上げ後の機能追加
...
4. リスクレベルが大きく異なる要素
   - 高リスクPBI: 決済処理のアーキテクチャ変更
   - 低リスクPBI: UI改善

5. 異なるタイミングでリリース可能な要素
   - 先行PBI: バグ修正（即座にリリース）
   - 後続PBI: 機能追加（来月リリース）
```
...
### Q1: 複数のタイプに該当する場合（複合型PBI）はどうすればよいですか？

**A1:** 複合型PBIは実務で頻繁に発生します。以下の優先順位判定ルールに従ってください：

#### 📌 優先順位ルール（上位ほど優先）

```
1. HOT (Hotfix) - 緊急性が最優先
2. NPI (New Project) - プロジェクト新規立ち上げは他の全てに優先
3. ARC (Architecture Improvement) - 根本的な設計変更
4. NFD (New Feature) - 新しいユースケースの追加
5. ENH (Enhancement) - 既存機能の改善
6. REF (Refactoring) - コード品質改善
7. BUG (Bug Fix) - バグ修正
8. POC (Proof of Concept) - 実験的機能
```

#### 判定方法

**ステップ1**: 上記の優先順位で主タイプを決定
**ステップ2**: 副次的な要素を記録
**ステップ3**: 実行フェーズを決定（主タイプの標準フェーズを基本とする）

#### 複合型PBIの具体例

**例1: バグ修正 + アーキテクチャ改善**
```
【PBI】決済処理のバグ修正とアーキテクチャ見直し
- バグ: 決済失敗時のリトライロジックに不具合
- 改善: 決済処理全体をイベント駆動アーキテクチャに移行

【判定】ARC (Architecture Improvement) ← 優先順位3
【理由】根本的な設計変更を伴うため、ARCを主タイプとする
【副次要素】BUG（バグ修正も含む）
【実行フェーズ】Phase 0 → 2 → 3 → 4 → 5（全設計フェーズ実施）
【対応方針】
- Phase 2: 新アーキテクチャ設計に既存バグ対策を含める
- Phase 3: 新実装と同時にバグ修正を実施
- Phase 4: 全要素を含めて統合テスト
```

**例2: 機能追加 + リファクタリング**
```
【PBI】通知機能の追加と既存コードのリファクタリング
- 機能追加: メール通知機能の実装
- リファクタリング: 通知関連の既存コードを整理

【判定】NFD (New Feature Development) ← 優先順位4
【理由】新しいユースケース（メール通知）追加が主目的
【副次要素】REF（リファクタリングも含む）
【実行フェーズ】Phase 0 → 2 → 3 → 4 → 5
【対応方針】
- Phase 2: 新機能設計時に既存コードのリファクタリング方針も決定
- Phase 3: 新機能実装と同時にリファクタリング実施
```

**例3: 緊急バグ修正 + 根本原因の設計改善**
```
【PBI】本番障害の緊急対応と根本原因対策
- 緊急対応: メモリリークによるサービスダウン（即座の修正必要）
- 根本対策: メモリ管理アーキテクチャの見直し

【判定】HOT (Hotfix) → 後日 ARC ← 優先順位1（緊急性優先）
【理由】緊急性が最優先。根本対策は別PBIで実施
【実行フェーズ】
  - HOT: 簡易Phase 0 → Phase 3 → 簡易Phase 4 → Phase 5（24時間以内）
  - ARC: 後日、別PBI（PROJ-XXXX-2）として Phase 0 → 2 → 3 → 4 → 5
【対応方針】
- まずHOTで応急処置（メモリリーク箇所の修正のみ）
- Phase 6のレビュー後、根本原因対策を別PBIとして起票
- 根本対策PBIはARCとして実施
```
...
- ✅ BUG: 決済失敗時のバグ修正
- ✅ REF: 既存コードのリファクタリング
- ⏭️ ENH: UI改善（別PBIとして切り出し）

### 判定理由
根本的なアーキテクチャ変更（同期処理→イベント駆動）を伴うため、
ARCを主タイプとして全設計フェーズを実施。
バグ修正とリファクタリングは実装フェーズで併せて対応。
...
### 実行フェーズ
Phase 0 → 2 → 3 → 4 → 5（全フェーズ実施）

### 副次要素の扱い
- **Phase 2**: 新アーキテクチャ設計に既存バグ対策を含める
- **Phase 3**: 新実装と同時にバグ修正、リファクタリング実施
- **Phase 4**: 全要素を含めて統合テスト
- **Phase 5**: まとめてデプロイ

### 別PBIとして切り出した要素
- **PROJ-1235**: UI改善（ENH）
  - 理由: アーキテクチャ変更と独立して実施可能
  - タイミング: PROJ-1234完了後に実施
```

#### PBI分割の判断基準

以下の場合は、PBIを分割することを推奨します：
