...
本フェーズでは、**作業のたびに** 監査ログ（structured_output）を記録すること。

- **フェーズ開始時**: 本フェーズの目的・対象・計画を structured_output に記録する
- **作業のたびに**: 重要なサブタスクの開始・完了、意思決定、前提変更があった時点で即時更新する
- **フェーズ完了時**: 成果物サマリ・残課題・次フェーズへの引き継ぎを記録する

詳細は [AI-WORKLOG-ENFORCEMENT-GUIDE.md](../02-ai-guides/AI-WORKLOG-ENFORCEMENT-GUIDE.md) の「3.1 更新の必須タイミング」「3.3 フェーズ／イベント別更新タイミング」に従うこと。
