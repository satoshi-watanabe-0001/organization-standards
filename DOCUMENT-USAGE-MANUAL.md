...
本組織では、以下の**3層の標準**が存在します：

```
┌─────────────────────────────────────────────────────────┐
│  Layer 1: 組織標準（Organization Standards）             │
│  ├─ 適用範囲: 組織全体（全プロジェクト・全チーム）          │
│  ├─ 場所: devin-organization-standards リポジトリ        │
│  └─ 承認: Architecture Review Committee                │
├─────────────────────────────────────────────────────────┤
│  Layer 2: プロジェクト標準（Project Standards）           │
│  ├─ 適用範囲: 特定プロジェクト内のみ                       │
│  ├─ 場所: プロジェクトリポジトリ（docs/standards/）       │
│  └─ 承認: Engineering Manager + Architect              │
├─────────────────────────────────────────────────────────┤
│  Layer 3: チーム標準（Team Standards）                   │
│  ├─ 適用範囲: 特定チーム内のみ                            │
│  ├─ 場所: チームリポジトリ（TEAM_STANDARDS.md）          │
│  └─ 承認: Engineering Manager                          │
└─────────────────────────────────────────────────────────┘
```
...
**対象領域**:
- セキュリティ・コンプライアンス（`07-security-compliance/`）
- クリティカルインフラストラクチャ
- 組織プロセス（インシデント対応、変更管理）

**組織標準の役割**: 必須基準（例外は極めて困難）
...
[ステップ2] 組織標準のレベルを確認
    ├─ 「必須」レベル → 組織標準を採用
    ├─ 「推奨」レベル → プロジェクト/チームで判断可能
    └─ 「参考」レベル → プロジェクト/チームで決定
    ↓
[ステップ3] 既存の矛盾解決ルールを適用
    1. 領域別優先度ルール（上記）
    2. 具体的 > 抽象的
    ...
    ↓
[ステップ4] それでも不明な場合
    ├─ Engineering Manager に相談
    ├─ 重要な判断 → Architect にエスカレーション
    └─ 組織横断的影響 → Architecture Review Committee
    ↓
判断を文書化（技術選定書、ADR等）
```
...
| 領域 | 優先度 | 組織標準の役割 | 例外の難易度 | 承認者 |
|------|--------|--------------|------------|--------|
| 🔒 セキュリティ・コンプライアンス | 組織 >>> 他 | 必須基準 | 極めて困難 | CTO + Legal + Security |
| 🔒 クリティカルインフラ | 組織 >> プロジェクト > チーム | 必須基準 | 困難 | Architecture Review Committee |
| ⚖️ API・アーキテクチャ | プロジェクト > 組織 ≈ チーム | ガイドライン | 容易 | Eng Manager + Architect |
| ⚖️ 技術スタック（承認済みリスト内） | プロジェクト > 組織 ≈ チーム | 承認リスト | 容易 | Eng Manager + Architect |
| ⚖️ 品質基準 | プロジェクト > 組織 ≈ チーム | 推奨値 | 容易 | Eng Manager |
| 🎨 コーディングスタイル | チーム > プロジェクト > 組織 | 参考情報 | 非常に容易 | Eng Manager（報告） |
| 🎨 開発ツール | チーム >>> 他 | 推奨情報 | 不要 | 承認不要 |
| 🎨 ワークフロー詳細 | チーム > プロジェクト > 組織 | 参考情報 | 非常に容易 | Eng Manager（報告） |
...
**完全な優先度設定ガイド**: `10-governance/standards-hierarchy-and-precedence.md`  
...
### ケース8: 標準間の矛盾がある（階層問題）⭐NEW

**状況**:
```
組織標準、プロジェクト標準、チーム標準で異なる記載がある。
どれを優先すべきか分からない。
```
...
```markdown
1. 該当する標準がどの領域か確認:
   - セキュリティ・コンプライアンス関連？ → 統制必須領域
   - API・アーキテクチャ・技術選定関連？ → バランス領域
   - スタイル・ツール・ワークフロー関連？ → 柔軟性領域
```

**ステップ2: 優先度マトリクスを参照**
```markdown
2. 上記「優先度マトリクス」で該当領域の優先度を確認
```
...
3. 判断結果を記録:
   - 技術選定書（技術スタック関連）
   - ADR（アーキテクチャ決定）
   - チーム標準ドキュメント（スタイル・ツール関連）
```

**例**:
```yaml
質問: "組織標準は2スペースだが、チームは4スペースを使いたい"

回答:
  領域: 柔軟性重視領域（コーディングスタイル）
  優先度: チーム > プロジェクト > 組織
  結果: チーム標準（4スペース）を採用
  承認: Engineering Managerに報告（形式的）
  記録: TEAM_STANDARDS.md に記載
```
...
2. **structured_output を初期化**（`AICQ_AUDIT_LOG_SCHEMA.md` で定義された AICQ スキーマ＋Playbook を適用）
3. **messages＋structured_output の回収計画を確立**（誰が／いつ／どのAPIで回収し、どの命名規則で保存するか）

**セッション中の必須運用**
- **各フェーズにおいて、作業のたびに監査ログを記録すること。** 作業の重要・軽微を問わず、すべての作業を記録対象とする。フェーズ開始・完了に加え、重要なサブタスク・意思決定のたびに structured_output を更新し、漏れを許さない。詳細は [AI-WORKLOG-ENFORCEMENT-GUIDE.md](00-guides/02-ai-guides/audit-log/AI-WORKLOG-ENFORCEMENT-GUIDE.md) のセクション3.1・3.3を参照すること。

**参照（必須）**
- [AI-WORKLOG-ENFORCEMENT-GUIDE.md](00-guides/02-ai-guides/audit-log/AI-WORKLOG-ENFORCEMENT-GUIDE.md)
- [AICQ_AUDIT_LOG_SCHEMA.md](00-guides/02-ai-guides/audit-log/AICQ_AUDIT_LOG_SCHEMA.md)
- [AICQ_Devin_reasoning_log.md](00-guides/02-ai-guides/audit-log/AICQ_Devin_reasoning_log.md)
- [AI-WORKLOG-IMPLEMENTATION-GUIDE.md](00-guides/02-ai-guides/audit-log/AI-WORKLOG-IMPLEMENTATION-GUIDE.md)
- [AI-AUDIT-LOG-PROTOCOL.md](00-guides/02-ai-guides/audit-log/AI-AUDIT-LOG-PROTOCOL.md)

**暫定運用（AuditLog クライアント未整備時）**

- AuditLog クライアント／SDK がまだ存在しない場合は、  
  `AI-AUDIT-LOG-PROTOCOL.md` セクション6.3に従い、自律型AI自身（またはオーケストレータ）がツール群を用いて  
  セッションごとに `session_dump.json` / `structured_output.json` / `messages.jsonl` を直接出力してください。
...
3.5. **監査ログ記録の確認（必須）**
   - 本フェーズでは **作業のたびに** structured_output を更新する
   - AI-WORKLOG-ENFORCEMENT-GUIDE.md の「3.1 更新の必須タイミング」「3.3 フェーズ／イベント別更新タイミング」を満たすよう、フェーズ開始時・完了時および重要サブタスク・意思決定のたびに記録する
...
1.5. **監査ログの最終更新**
   - フェーズ完了時の内容を structured_output に反映したか確認する
   - 漏れがあれば即時追記する
