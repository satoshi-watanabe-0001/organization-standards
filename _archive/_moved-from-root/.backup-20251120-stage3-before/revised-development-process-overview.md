...
change_type: "minor - Phase 5 (旧Phase 2B)並行実施の具体的スケジュール追加"
changelog:
  - version: "3.4.0"
    date: "2025-11-20"
  ...
  - version: "3.3.0"
    date: "2025-11-19"
    changes: "Phase 5 (旧Phase 2B)並行実施の詳細タイムライン、トリガー、FAQを追加"
  - version: "3.2.0"
    date: "2025-11-19"
    changes: "Phase 2A/2B リネーム対応（Phase 2.1/2.2から変更）"
---
...
**変更後**: Phase 2A (事前設計) + Phase 5 (旧Phase 2B) (詳細設計) - 実行順序を明確化
...
重要な注意:
  ...
  phase_2_2:
    phase_number: "Phase 5 (旧Phase 2B)"
    execution_order: "Phase 4 の後、Phase 6 (旧Phase 5) の前"
    timing: "実装完了後 (または Phase 3-4 と並行)"
```

**Phase 5 (旧Phase 2B) は Phase 4 の後に実行されます**
...
**移行期間**: 2025-11-20 〜 2025-12-20
- 移行期間中は旧番号を併記します（例: Phase 5 (旧Phase 2B)）
- 2025-12-20以降は新番号のみを使用します

**重要**: 実行順序は変更されていません。Phase 5（旧Phase 2B）は引き続きPhase 4の後に実行されます。
...
5. Phase 4: レビュー・QA (Review & QA)
   期間: 3-5日
   ↓
6. Phase 5 (旧Phase 2B): 詳細設計 (Post-Implementation Design) ★NEW
   期間: 2-3日
   目的: 実装内容の文書化と設計の整理
   別名: Detailed Design
   実行順序: Phase 4 の後（または Phase 3-4 と並行）
   ↓
7. Phase 6 (旧Phase 5): デプロイメント (Deployment)
   期間: 数時間～1日
   ↓
8. Phase 7 (旧Phase 6): 運用・保守 (Operations & Maintenance)
   期間: 継続的
```
...
Phase 0 → Phase 1 → Phase 2A (事前設計 1-2日)
                   ...
                   Phase 4 (レビュー・QA 3-5日)
                        ↓
              Phase 5 (旧Phase 2B) (詳細設計 2-3日)
              ※Phase 3-4 と並行実施も可能
                        ↓
                   Phase 6 (旧Phase 5) → Phase 7 (旧Phase 6)
```

**タイミング詳細**:
- Phase 2A: 実装開始前に実施
- Phase 5 (旧Phase 2B): Phase 3 実装中に並行開始、Phase 4 完了時に完成
...
**タイミング詳細**:
- Phase 2A: 実装開始前
- Phase 5 (旧Phase 2B): 実装開始前に詳細設計を完成
- Phase 5 (旧Phase 2B) 更新: 実装完了後に最終版に更新
...
**タイミング詳細**:
- Phase 2A: スキップ
- Phase 5 (旧Phase 2B): Phase 4 完了後に実装内容を文書化
...
## 📝 Phase 5 (旧Phase 2B): 詳細設計 (Post-Implementation Design)
...
execution_order:
  ...
  before: "Phase 6 (旧Phase 5) (デプロイメント)"
  timing_options:
...
重要:
  "Phase 5 (旧Phase 2B) という番号だが、実行順序は Phase 4 の後"
...
> **2025-11-19 追加**: Phase 5 (旧Phase 2B) 並行実施の具体的なタイムラインとトリガーを追加

#### 並行実施とは

Phase 3（実装）とPhase 4（レビュー・QA）と「並行」してPhase 5 (旧Phase 2B)（詳細設計）を
進めることで、以下のメリットを実現:
...
├──────────────────────────────────────────────────────────┤
│ Week 2: Phase 3 (実装継続) + Phase 5 (旧Phase 2B) 開始                │
├──────────────────────────────────────────────────────────┤
│ Day 1-2 │ 実装  80% │ 設計 20% │ ★Phase 5 (旧Phase 2B)開始              │
│         │           │          │ コア機能ドキュメント化開始   │
│ Day 3-5 │ 実装  70% │ 設計 30% │ API仕様書作成開始          │
│         │           │          │ 追加機能実装継続            │
├──────────────────────────────────────────────────────────┤
│ Week 3: Phase 4 (レビュー・QA) + Phase 5 (旧Phase 2B) 継続            │
├──────────────────────────────────────────────────────────┤
...
│         │    60%    │          │                           │
├──────────────────────────────────────────────────────────┤
│ Week 3 終了後: Phase 5 (旧Phase 2B) 完了                              │
├──────────────────────────────────────────────────────────┤
...
**Phase 5 (旧Phase 2B) 開始トリガー** (Week 2 Day 1):
- [ ] コア機能（CRUD操作）が実装完了
- [ ] 主要APIエンドポイントが動作確認済み
- [ ] ユニットテストが基本ケースをカバー
...
├──────────────────────────────────────────────────────────┤
│ Week 3: Phase 3 (実装継続) + Phase 5 (旧Phase 2B) 開始                │
├──────────────────────────────────────────────────────────┤
│         │ 実装  80% │ 設計 20% │ ★Phase 5 (旧Phase 2B)開始              │
│         │           │          │ コア機能ドキュメント化       │
│         │           │          │ 追加機能実装                │
├──────────────────────────────────────────────────────────┤
│ Week 4: Phase 3 (実装完了) + Phase 5 (旧Phase 2B) 継続                │
├──────────────────────────────────────────────────────────┤
│         │ 実装  70% │ 設計 30% │ 最終機能実装               │
│         │           │          │ API仕様書、データモデル作成  │
├──────────────────────────────────────────────────────────┤
│ Week 5: Phase 4 (レビュー・QA) + Phase 5 (旧Phase 2B) 継続            │
├──────────────────────────────────────────────────────────┤
│         │ レビュー  │ 設計 40% │ テスト・レビュー実施        │
│         │    60%    │          │ アーキテクチャ図作成        │
├──────────────────────────────────────────────────────────┤
│ Week 5 終了後: Phase 5 (旧Phase 2B) 完了                              │
└──────────────────────────────────────────────────────────┘
```

**Phase 5 (旧Phase 2B) 開始トリガー** (Week 3):
- [ ] 実装期間の40-50%が経過
- [ ] コア機能がすべて実装完了
- [ ] 統合テストが基本シナリオをカバー
- [ ] アーキテクチャの大きな変更がない
...
```
 ...
 YES                        NO
   ...
   ▼                         ▼
┌─────────────────┐ ┌──────────────────┐
│ Phase 5 (旧Phase 2B) 開始可 │ │ もう少し実装継続  │
│ トリガー確認へ  │ │ 1-2日後に再判断   │
└────────┬────────┘ └──────────────────┘
         │
         ▼
┌───────────────────────────────────┐
│ Phase 5 (旧Phase 2B) 開始トリガー（確認）     │
├───────────────────────────────────┤
│ □ コア機能（CRUD操作）完了        │
│ □ 主要APIが動作確認済み           │
│ □ データモデルが確定              │
...
午後: Phase 5 (旧Phase 2B) に切り替え（ドキュメント作成は午後が適）
```
...
**タイミング2: 待ち時間活用**（最も効率的）
```
Phase 4 のテスト実行中、CI/CD実行中 → Phase 5 (旧Phase 2B) 作業
Phase 4 のレビュー待ち → Phase 5 (旧Phase 2B) 作業
```
...
**Q1: Week 1から Phase 5 (旧Phase 2B) を開始してはいけないのか?**

A1: 推奨しません。理由:
- Week 1 はプロジェクト構造構築、コア実装が最優先
- この段階ではアーキテクチャが変更される可能性が高い
- Phase 5 (旧Phase 2B) を開始しても、後で大きく書き直すことになる
- → Week 2 からの開始が最適

**Q2: Phase 3 が予定より早く終わった場合は?**

A2: Phase 5 (旧Phase 2B) に集中するチャンス:
- 残り時間を Phase 5 (旧Phase 2B) に100%投入
- 早期にドキュメントを完成させる
- Phase 4 (レビュー) の前にドキュメントレビューも完了可能

**Q3: Phase 3 が遅れている場合は?**

A3: Phase 5 (旧Phase 2B) 開始を遅らせる:
- まずPhase 3 の完了を優先
- Phase 5 (旧Phase 2B) は Phase 4 期間中に集中して実施
- Phase 4 完了後に Phase 5 (旧Phase 2B) を単独で実施（2-3日）
- リードタイムは伸びるが、品質は確保

**Q4: AIエージェントに Phase 5 (旧Phase 2B) を自動化できるか?**
...
Week 1-2: Phase 3 (実装)
  ├─ メイン作業: 実装 (80%)
  └─ 並行作業: Phase 5 (旧Phase 2B) 開始 (20%)
      - 完成部分から順次文書化
      - アーキテクチャ図の作成開始
      ↓
Week 3: Phase 4 (レビュー・QA)
  ├─ メイン作業: レビュー・テスト (70%)
  └─ 並行作業: Phase 5 (旧Phase 2B) 継続 (30%)
      - API仕様書の完成
      - データモデル文書の作成
      ↓
Week 3 終了時: Phase 5 (旧Phase 2B) 完了
  └─ 全ての設計ドキュメントが完成
      ↓
Week 4: Phase 6 (旧Phase 5) (デプロイメント)
```
...
Week 0: Phase 2A (事前設計 1-2日)
  └─ 成果物: ADR, API契約, 制約条件
      ↓
Week 1: Phase 5 (旧Phase 2B) (詳細設計 3-5日) ※実装前
...
Week 4: Phase 4 (レビュー・QA)
  └─ 実装との整合性確認
      ↓
Week 4 終了時: Phase 5 (旧Phase 2B) 更新 (1日)
  └─ 実装との差分を反映し、最終版を作成
      ↓
Week 5: Phase 6 (旧Phase 5) (デプロイメント)
```
...
Week 3: Phase 4 (レビュー・QA)
  └─ テストとレビュー
      ↓
Week 3-4: Phase 5 (旧Phase 2B) (詳細設計 1-2日) ※実装後
  └─ 実装された内容を文書化 (簡易版)
      ↓
Week 4: Phase 6 (旧Phase 5) (デプロイメント)
```
...
4. Phase 5 (旧Phase 2B) (並行 or 直後) → 設計書, API仕様書, 図, データモデル
5. Phase 6 (旧Phase 5) → デプロイ
...
**実行順序**:
1. Phase 2A (1-2日) → ADR, API契約, 制約条件
2. Phase 5 (旧Phase 2B) (3-5日) → 詳細設計 (実装前に完成)
3. Phase 3 (1-4週間) → 実装
4. Phase 4 (3-5日) → レビュー・QA
5. Phase 5 (旧Phase 2B) 更新 (1日) → 最終版に更新
6. Phase 6 (旧Phase 5) → デプロイ
...
3. Phase 5 (旧Phase 2B) (1日) → 簡易版設計書 (必要に応じて)
4. Phase 6 (旧Phase 5) → デプロイ
...
実行順序の覚え方:
  "Phase 2A → Phase 3 → Phase 4 → Phase 5 (旧Phase 2B) → Phase 6 (旧Phase 5)"
...
1. **AI-MASTER-WORKFLOW-GUIDE.md** の改訂
   - Phase 2A / Phase 5 (旧Phase 2B) の説明追加
   - 実行順序の明確化
