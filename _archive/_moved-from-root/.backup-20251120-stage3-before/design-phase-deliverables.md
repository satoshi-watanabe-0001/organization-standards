...
phase_2a:
  ...
  execution_order: "Phase 2A → Phase 3 → Phase 4 → Phase 5 (旧Phase 2B)"
  purpose: "実装の方向性を定める"
  detail_level: "30-40%"
...
**重要**: Phase 5 (旧Phase 2B)は番号が「2」だが、実際の実行順序はPhase 4の後です。
...
| 項目 | Phase 2A | Phase 5 (旧Phase 2B) | 違い |
|------|----------|---------|------|
| **目的** | 実装方針合意 | 完全ドキュメント化 | 目的が異なる |
| **詳細度** | 30-40% | 100% | Phase 5 (旧Phase 2B)は2.5倍詳細 |
| **図の数** | 2-3図 | 6-10図 | Phase 5 (旧Phase 2B)は3-4倍多い |
| **C4レベル** | Level 1-2 | Level 1-4 | Phase 5 (旧Phase 2B)は全レベル |
| **シーケンス図** | なし | あり（3-5図） | Phase 5 (旧Phase 2B)で追加 |
| **デプロイ図** | なし | あり | Phase 5 (旧Phase 2B)で追加 |
| **作成時間** | 1-2時間 | 4-6時間 | Phase 5 (旧Phase 2B)は3-4倍 |
| **更新頻度** | 低（確定後変更少） | 中（実装に合わせて更新） | Phase 5 (旧Phase 2B)は変更追従 |
| **作成タイミング** | 実装前 | 実装後または並行 | タイミングが異なる |
| **形式** | 手書き可 | 専用ツール推奨 | Phase 5 (旧Phase 2B)はフォーマル |
...
| **詳細度** | 40-50% | 100% | Phase 5 (旧Phase 2B)は2倍詳細 |
| **エンドポイント数** | 3-5個 | 10-20個 | Phase 5 (旧Phase 2B)は3-4倍多い |
| **フォーマット** | Markdown | OpenAPI 3.0 | Phase 5 (旧Phase 2B)は標準形式 |
| **バリデーション** | 概要のみ | 詳細（正規表現含む） | Phase 5 (旧Phase 2B)は完全 |
| **エラーコード** | 主要3-5個 | 全20-30個 | Phase 5 (旧Phase 2B)は完全 |
| **使用例** | なし | あり（curl, Python等） | Phase 5 (旧Phase 2B)で追加 |
| **作成時間** | 2-3時間 | 4-6時間 | Phase 5 (旧Phase 2B)は2倍 |
| **コード生成** | 不可 | 可能 | Phase 5 (旧Phase 2B)のみ可能 |
| **作成タイミング** | 実装前 | 実装後または並行 | タイミングが異なる |
...
#### Phase 5 (旧Phase 2B): データモデル文書（完全版）
...
| **詳細度** | 30-40% | 100% | Phase 5 (旧Phase 2B)は2.5倍詳細 |
| **エンティティ数** | 3-5個 | 10-20個 | Phase 5 (旧Phase 2B)は3-4倍多い |
| **フィールド** | 主要のみ | 全フィールド | Phase 5 (旧Phase 2B)は完全 |
| **制約** | なし | 全制約 | Phase 5 (旧Phase 2B)で追加 |
| **インデックス** | なし | 全インデックス | Phase 5 (旧Phase 2B)で追加 |
| **マイグレーション** | なし | あり | Phase 5 (旧Phase 2B)で追加 |
| **作成時間** | 1時間 | 4-6時間 | Phase 5 (旧Phase 2B)は4-6倍 |
| **ER図** | 簡易図 | 完全ER図 | Phase 5 (旧Phase 2B)は詳細 |
| **作成タイミング** | 実装前 | 実装後または並行 | タイミングが異なる |
...
3. **作成時間**: Phase 5 (旧Phase 2B)の30-40%の時間
   - Phase 2A: 1-2日
   - Phase 5 (旧Phase 2B): 2-3日（またはPhase 3-4と並行）
...
Phase 5 (旧Phase 2B)では実装後に書くため、推測ではなく事実を記録します。

---

### Q3: Phase 5 (旧Phase 2B)を実装と並行して作成する場合、どのタイミングで書けばいいですか？
...
**パターン3: レビュー待ち時間に作成**
```
実装完了 → PRレビュー待ち → Phase 5 (旧Phase 2B)作成
→ 待ち時間を有効活用
```
...
### Q4: 小規模プロジェクトでもPhase 5 (旧Phase 2B)を100%完成させる必要がありますか？

**A**: プロジェクト規模に応じて調整可能です：

**小規模プロジェクト（~1ヶ月）**:
- Phase 5 (旧Phase 2B): 簡易版（60-70%の詳細度）
- 省略可能: デプロイメント図、詳細なシーケンス図
- 必須: 設計書、API仕様書、データモデル

**中規模プロジェクト（1-3ヶ月）**:
- Phase 5 (旧Phase 2B): 標準版（80-90%の詳細度）
- 推奨: ほぼ全ての成果物

**大規模プロジェクト（3ヶ月~）**:
- Phase 5 (旧Phase 2B): 完全版（100%の詳細度）
- 必須: 全ての成果物
...
### Q5: Phase 2Aの成果物とPhase 5 (旧Phase 2B)の成果物で重複がある場合、どうすればいいですか？

**A**: Phase 5 (旧Phase 2B)で統合・拡張します：

**例: アーキテクチャ図**
1. Phase 2A: 簡易図を作成（2-3図）
2. Phase 5 (旧Phase 2B): Phase 2Aの図を含む完全版を作成（6-10図）
   - Phase 2Aの図は「概要」セクションに配置
   - 詳細図を追加
3. 結果: Phase 5 (旧Phase 2B)のドキュメントに全てが含まれる

**例: API仕様**
1. Phase 2A: API契約書（Markdown、3-5エンドポイント）
2. Phase 5 (旧Phase 2B): 完全API仕様書（OpenAPI、10-20エンドポイント）
   - Phase 2Aの契約書は「基本契約」セクションに配置
   - 全エンドポイントを追加
3. 結果: Phase 5 (旧Phase 2B)のドキュメントに全てが含まれる

**原則**: Phase 5 (旧Phase 2B)が「完全版」として全てを含む最終成果物となります。
...
3. **目的が異なる**
   ...
   - Phase 5 (旧Phase 2B): 全ステークホルダーがシステムを理解できるように

4. **タイミングが異なる**
   - Phase 2A: 実装前
   - Phase 5 (旧Phase 2B): 実装後または並行

5. **作成方法が異なる**
   - Phase 2A: 手書き・簡易ツール可
   - Phase 5 (旧Phase 2B): 専用ツール・標準フォーマット推奨

---

このガイドラインにより、Phase 2AとPhase 5 (旧Phase 2B)の成果物の違いが明確になり、
...
| 1.1.0 | 2025-11-20 | Engineering Team | Phase 2A vs Phase 5 (旧Phase 2B) 成果物詳細度比較セクション追加 |
