...
このドキュメントは、改訂後の設計フェーズ (Phase 2A, Phase 5 (旧Phase 2B)) で「いつ、何を、誰が作るか」を一目で確認できるマトリクスです。
...
## 成果物マトリクス (Phase 2A vs Phase 5 (旧Phase 2B))

### 成果物の分類

| 成果物 | Phase 2A<br>実装前設計 | Phase 5 (旧Phase 2B)<br>実装後設計 | 目的の違い |
|--------|:--------------------:|:--------------------:|-----------|
...
| 成果物 | Phase 2A | Phase 5 (旧Phase 2B) | 作成時間 |
|--------|:--------:|:--------:|---------|
| ADR | ✓ (1-2個) | △ | 1-2時間 |
| API契約書 | △ | - | 1-2時間 |
| 制約条件文書 | ✓ | - | 1時間 |
...
**Phase 5 (旧Phase 2B) 合計**: 8-16時間 (1-2日)
...
**Phase 5 (旧Phase 2B) 合計**: 21-31時間 (3-4日、AI活用時 2-3日)
...
**Phase 5 (旧Phase 2B) 合計**: 36-57時間 (5-7日、AI活用時 3-4日)
...
| **Phase 3** | 実装 | - | コーディング |
| **Phase 5 (旧Phase 2B)** | 設計書 | 高 | 実装内容の文書化 |
| | 完全版API仕様書 | 高 | 全エンドポイント仕様 |
| | アーキテクチャ図 | 高 | 確定版の図 |
| | データモデル文書 | 高 | ER図、スキーマ |

**推奨パターン**: AI最適化型 (Phase 2A → Phase 3 → Phase 5 (旧Phase 2B)並行)
...
| | 制約条件文書 | 高 | 後方互換性要件 |
| **Phase 3** | 実装 | - | コーディング |
| **Phase 5 (旧Phase 2B)** | 設計書 | 高 | 変更内容の文書化 |
...
| | 制約条件文書 | 高 | 共通理解の確立 |
| | 技術提案書 | 高 | ステークホルダー承認 |
| **Phase 5 (旧Phase 2B)** | 設計書 (事前作成) | 高 | 実装前の実装後設計 |
| **(実装前)** | 完全版API仕様書 | 高 | 契約の詳細化 |
| **Phase 3** | 実装 | - | 各チームが実装 |
| **Phase 5 (旧Phase 2B)** | 設計書 (更新) | 中 | 最終版の整備 |
| **(実装後)** | アーキテクチャ図 | 高 | 全体構成の確定 |

**推奨パターン**: 従来型 (Phase 2A → Phase 5 (旧Phase 2B)詳細 → Phase 3)
...
| **Phase 3** | 実装 | - | コーディング |
| **Phase 5 (旧Phase 2B)** | 設計書 (更新) | 低 | 変更点のみ記録 |
| | 更新版ADR | △ | 重要な決定があれば |

**推奨パターン**: リバースエンジニアリング型 (Phase 3 → Phase 5 (旧Phase 2B)簡易版)
...
Week 1-2: Phase 3 実装 + Phase 5 (旧Phase 2B) 並行開始
  ├─ 実装: メイン作業
  └─ Phase 5 (旧Phase 2B): 完成部分から順次文書化 (20-30%の時間)

Week 3: Phase 4 レビュー + Phase 5 (旧Phase 2B) 完成
  ├─ レビュー: メイン作業
  └─ Phase 5 (旧Phase 2B): 残りの文書化完了 (30-40%の時間)

Week 4: Phase 5 デプロイ
```

**リードタイム**: 4週間
**Phase 5 (旧Phase 2B)作業時間**: Phase 3-4期間に分散
...
```
Week 0-1: Phase 2A (1-2日) + Phase 5 (旧Phase 2B) (3-5日)
  ├─ Phase 2A: ADR, API契約, 制約条件
  └─ Phase 5 (旧Phase 2B): 実装後設計 (実装前)

Week 2-3: Phase 3 実装

Week 4: Phase 4 レビュー + Phase 5 (旧Phase 2B) 更新 (1日)

Week 5: Phase 5 デプロイ
```

**リードタイム**: 5週間
**Phase 5 (旧Phase 2B)作業時間**: 実装前に集中 + 実装後に更新
...
Week 3-4: Phase 5 (旧Phase 2B) (2-3日)
  └─ 実装完了後に文書化
...
### Phase 2A → Phase 3 → Phase 5 (旧Phase 2B) の流れ
...
ステップ5: Phase 5 (旧Phase 2B) の実施 (並行 or 実装後)
├─ 設計書を作成
├─ AI活用で効率化
└─ レビュー・承認
```
...
- **Phase 5 (旧Phase 2B)**: 実装された内容を正確に文書化
- **効果**: 手戻り削減、並行作業、リードタイム短縮

---

### Q2: すべてのプロジェクトで Phase 2A と Phase 5 (旧Phase 2B) の両方が必要ですか?
...
### Q3: Phase 5 (旧Phase 2B) は並行と実装後のどちらが良いですか?
