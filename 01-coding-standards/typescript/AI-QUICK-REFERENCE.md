...
- **参照**: [typescript-inline-comment-examples.md](typescript-inline-comment-examples.md)

#### 27. ✅ WHY原則の遵守
```typescript
// ✅ OK: なぜその実装をしているか説明
// NOTE: IE11サポートのため、古いPromise構文を使用
 ...
 * ユーザー権限の検証
 * 
 * NOTE: 複数の条件を組み合わせて権限を判定
 * - 管理者: すべてのリソースへアクセス可
 * - マネージャー: 自部署のリソースへアクセス可
 * - 一般ユーザー: 自分のリソースのみアクセス可
 * 
 * 複雑度: 12 (基準10以上のため詳細コメント必須)
 */
function checkPermission(user: User, resource: Resource): boolean {
  // 実装...
}
```
- **必須**: 循環的複雑度10以上の関数に詳細コメント
- **参照**: [typescript-inline-comment-examples.md](typescript-inline-comment-examples.md)
...
/**
 * 【テスト対象】ユーザー作成関数
 * 【テストケース】有効なメールアドレスでユーザーを作成
 * 【期待結果】ユーザーIDが返され、DBに保存される
 * 【ビジネス要件】ユーザー登録機能（REQ-USER-001）
 */
describe('createUser', () => {
...
- **参照**: [typescript-test-comment-examples.md](typescript-test-comment-examples.md)

#### 30. ✅ TODO/FIXME/HACKの書式
```typescript
// TODO: [山田太郎] [期限: 2025-12-31] 理由: キャッシュ機能を追加（パフォーマンス改善）
// FIXME: [佐藤花子] [期限: 2025-11-30] 理由: nullチェックが不足（NPE対策）
// HACK: [鈴木一郎] 理由: ライブラリのバグ回避のための暫定対応（issue #456参照）
```
- **必須**: 担当者、期限、理由を記載
- **参照**: [typescript-inline-comment-examples.md](typescript-inline-comment-examples.md)
...
| **コメント規約** | [typescript-inline-comment-examples.md](typescript-inline-comment-examples.md) |
| **テストコメント** | [typescript-test-comment-examples.md](typescript-test-comment-examples.md) |

---

**AI-QUICK-REFERENCE** - Phase 7  
作成日: 2025-11-13  
更新日: 2025-11-15（コメント規約追加）  
